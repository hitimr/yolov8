...
class EarlyStopping:
    ...
    def __init__(self, patience=50, stop_event=None):
        ...
        Args:
            ...
            stop_event (threading.Event or multiprocessing.Event, optional): If set and becomes triggered, training will stop.
        """
        self.best_fitness = 0.0  # i.e. mAP
        self.best_epoch = 0
        self.patience = patience or float("inf")  # epochs to wait after fitness stops improving
        self.possible_stop = False  # flag for imminent stop
    ...
    def __call__(self, epoch, fitness):
        ...
        """
        if fitness is None:  # e.g. if validation was skipped
            return False
        ...
        # How many epochs since last improvement?
        ...
        if stop:
            ...
            LOGGER.info(
                f"{prefix}Training stopped early as no improvement observed in the last {self.patience} epochs. "
                f"Best results were at epoch {self.best_epoch} (fitness={self.best_fitness}).\n"
                f"To adjust, pass a new patience value, e.g. `EarlyStopping(patience=100)` or disable with `patience=0`."
            )

        # Only check stop_event if it was provided
